...
class Particle:
    ...
    def update_phase(self, lp):
        # v1.0 Standardized Thresholds
        strong_th = self.mat['alpha'] * 1.5 - self.mat['beta'][1] * 0.05
        core_th = self.mat['alpha'] * 3.0
        
        if lp > core_th: 
            self.phase = "Core"; self.p_amp = 60.0; self.sigma = 6.0
        elif lp > strong_th: 
            self.phase = "Strong"; self.p_amp = 20.0; self.sigma = 12.0
        elif lp > 5.0: 
            self.phase = "Composite"; self.p_amp = 10.0; self.sigma = 18.0
        else: 
            self.phase = "Stable"; self.p_amp = 2.0; self.sigma = 35.0
...
class TSHCore:
    ...
    def _compute_forces(self):
        for p in self.particles:
            ...
            p.acc = (f_unified * p.mass + np.array([0, 0.5])) / p.mass

    def _integrate(self):
        for p in self.particles:
            p.vel += p.acc * DT; p.pos += p.vel * DT
            p.pos[0] = np.clip(p.pos[0], 0, WIDTH); p.pos[1] = np.clip(p.pos[1], 0, HEIGHT)
            p.vel *= 0.99
